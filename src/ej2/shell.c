...
int main(int argc, char *argv[]) {
    ...
    // Modo no interactivo con -c (para tests automatizados)
    if (argc > 2 && strcmp(argv[1], "-c") == 0) {
        strcpy(command, argv[2]);
        command[strcspn(command, "\n")] = '\0';
        
        // Procesa el comando una vez y sale
        goto process_command; // Uso de goto para reutilizar el código existente
    }

    while (1) {
        printf("Shell> ");
        
        // Verifica EOF (fin de archivo)
        if (fgets(command, sizeof(command), stdin) == NULL) {
            printf("\n");
            break;  // Sale del bucle si detecta EOF (Ctrl+D)
        }
...
process_command:
        ...
        // Si estamos en modo no-interactivo (-c), salimos después de procesar el comando
        if (argc > 2 && strcmp(argv[1], "-c") == 0) {
            break;
        }
    }
