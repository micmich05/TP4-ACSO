...
//     if (n < 3 || start > n) { //agregue breve chequeo
//         printf("Entradas inválidas\n");
//         exit(EXIT_FAILURE);
//     }
...
//     int total = n + 1; // padre + n hijos
//     int (*pipes)[2] = malloc(sizeof(int[2]) * total); // matriz de pipes: pipes[i] conecta ring_procs[i] -> ring_procs[(i+1)%(n+1)]
...
//     int ring_idx;
//     if (pid > 0) {
//         ring_idx = 0; // estamos en el padre
//     } else {
//         ring_idx = ((myid - start + n) % n) + 1; // posicion del anillo
//     }

//     for (int i = 0; i < total; ++i) { /* Cierro los pipes que no me corresponden */
//         if (i == ring_idx) {
//             close(pipes[i][0]); // mi pipe de salida: me quedo con [1], cierro [0]
//         } else if (i == (ring_idx - 1 + total) % total) {
//             close(pipes[i][1]); // mi pipe de entrada: me quedo con [0], cierro [1]
//         } else {
//             close(pipes[i][0]); // cierro ambos extremos
//             close(pipes[i][1]);
//         }
//     }

//     if (ring_idx == 0) {
//         write(pipes[0][1], &buffer[0], sizeof(buffer[0])); /* === PADRE === */
//         close(pipes[0][1]);
        
//         int result;
//         read(pipes[total-1][0], &result, sizeof(result)); // recibo el resultado final
//         close(pipes[total-1][0]);
...
int main(int argc, char **argv)
{
    ...
    if (pid > 0) { //para el padre
        ...
        ring_idx = ((myid - start + n) % n) + 1; // valores 1..n
    }
