...
int main(int argc, char **argv)
{
    ...
    if (pid > 0) { //para el padre
        ...
        // Corrección: el proceso 'start' debe tener ring_idx = 1
        ring_idx = ((myid - (start - 1) + n) % n) + 1; // valores 1..n
    }
    ...
    if (ring_idx == 0) { //padre
        // envío el mensaje inicial al proceso que tiene ring_idx = 1 (proceso 'start')
        // que lee de pipes[0]
        write(pipes[0][1], &buffer[0], sizeof(buffer[0]));
        close(pipes[0][1]);
